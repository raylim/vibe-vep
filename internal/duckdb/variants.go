...
func (s *Store) WriteVariantResults(results []VariantResult) error {
	...
	for _, r := range deduped {
		...
		if err := appender.AppendRow(
			...
			a.IsCanonicalMSK, a.IsCanonicalEnsembl, a.Allele, a.Biotype, a.ExonNumber, a.IntronNumber,
...
func (s *Store) LookupVariant(chrom string, pos int64, ref, alt string) ([]*annotate.Annotation, error) {
	rows, err := s.db.Query(`SELECT
		...
		is_canonical_msk, is_canonical_ensembl, allele, biotype, exon_number, intron_number,
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&ann.IsCanonicalMSK, &ann.IsCanonicalEnsembl, &ann.Allele, &ann.Biotype, &ann.ExonNumber, &ann.IntronNumber,
